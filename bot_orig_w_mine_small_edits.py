...
def error(bot, update, error):
	...
	bot.sendMessage(update.message.chat_id, text="Произошла ошибка при обработке этого сообщения (возможно юзверь не написал боту в личку, но подписался на обновления)", 
...
def sendnotif(bot, from_id, to_id, amount): # #zebest use of multiples! 
	if from_id != 0 and from_id in subscribed:
		capt = ''
		if to_id != 0:
			capt = 'для пользователя {}'.format(unames.get(to_id, 'Неизвестный юзверь {}'.format(to_id)))
		bot.sendMessage(from_id, text="У вас было отнято {} ₫ {}".format(amount, capt))
		del capt
	...
	if to_id != 0 and to_id in subscribed:
		...
		bot.sendMessage(to_id, text="Вам было добавлено {} ₫ {}".format(amount, capt))
...
def jobhourly(bot, job): # why args?
		...
		if data[2] == 'stop':
			if int(data[1]) == qfrom:
				bot.editMessageText(chat_id=chat_id, message_id=msg_id, reply_markup=InlineKeyboardMarkup([]),
					text="Сбор ₫ остановлен владельцем.")
			else:
				bot.answerCallbackQuery(callback_query_id=inlmsgid, text="Вы не владелец этого сбора ₫")
		else:
			if payment(chat_id, qfrom, int(data[1]), int(data[2]), True):
				bot.answerCallbackQuery(callback_query_id=inlmsgid, text="Успешно переведено {} ₫".format(data[2]))
				sendnotif(bot, qfrom, int(data[1]), int(data[2]))
			else:
				bot.answerCallbackQuery(callback_query_id=inlmsgid, text="Недостаточно ₫")
	else:
		return
...
def ask(bot, update, args):
	...
	bot.sendMessage(chat_id, text="{} просит {} ₫.{}".format(getuname(update.message.from_user), arg, captstr), 
		reply_markup=mrkup)
...
def pay(bot, update, args):
	...
	if not payment(chat_id, fromid, toid, arg, True):
		bot.sendMessage(chat_id, text="Недостаточно ₫!", reply_to_message_id=update.message.message_id)
	else:
		bot.sendMessage(chat_id, text="{} ₫ переведено.".format(arg), reply_to_message_id=update.message.message_id)
...
def subscr(bot, update):
	...
	if not from_user.id in subscribed:
		...
		bot.sendMessage(chat_id, text="""Успешно подписаны на обновления ₫.
...
def unsubscr(bot, update):
	...
	if from_user.id in subscribed:
		...
		bot.sendMessage(chat_id, text="Успешно отписаны от обновлений ₫.", 
			reply_to_message_id=update.message.message_id)
	else:
